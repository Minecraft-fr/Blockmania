...
public class GameManager {
  ...
  public void start() {
    ...
    for(Player player : Bukkit.getOnlinePlayers()) {
      ...
      new GameScoreboard(player.getUniqueId()).createScoreboard(player);
    ...
    for(Player player : Bukkit.getOnlinePlayers()) {
      GameScoreboard.updateTabScore(player, 0);
    }
    ...
    for(int x=-1; x<=1; x++) {
      for(int z=-1; z<=1; z++) { {
        zerozero.getRelative(x, 0, z).setType(Material.END_PORTAL);
      }}
    }
  }
  ...
  public void stop() {
    ...
    if(winners.size() == 1) {
      ...
      for(Player player : winners) {
        playersText.append(player.getDisplayName()).append(", ");
      }
      ...
      if(gamePlayerEntry.getValue().getScore() >= highestScore) {
        ...
        if(player != null) {
          ...
          if(Blockmania.getConfigManager().getTeamSize() != 1) {
            gameData.getPlayersTeam().get(player.getUniqueId()).getPlayersUUIDs().forEach(uuid -> {
              Player player1 = Bukkit.getPlayer(uuid);
              if(player1 != null) winners.add(player1);
            });
          }
        }
      }
