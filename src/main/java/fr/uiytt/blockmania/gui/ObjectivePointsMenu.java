...
public class ObjectivePointsMenu implements InventoryProvider {
	...
	public final SmartInventory inventory = SmartInventory.builder()
			.id("BM_ObjPoints")
			.title(ChatColor.DARK_GRAY + "Objective points")
			.size(3, 9)
			.provider(this)
	...
	public void init(Player player, InventoryContents contents) {
		...
		contents.set(1,1, ClickableItem.of(ItemStackBuilder.build(Material.RED_STAINED_GLASS_PANE, ChatColor.RED + "-10"), event -> {
			config.setObjectivePoints(Math.max(1, config.getObjectivePoints() - 10));
